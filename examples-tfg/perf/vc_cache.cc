...
template<typename T>
concept comparator = requires(T x) {
  { x.cmp(std::declval<int>(), std::declval<int>()) } -> bool;
};
...
void bubble_sort(std::vector<int>& xs, comparator virtual& c) {
